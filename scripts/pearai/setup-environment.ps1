...
function Initialize-BaseFunctionality {
    ...
    Set-Location .\extensions\grapefruitide-submodule
...
function Create-SymLink {
    Write-Host "`nCreating symbolic link 'extensions\grapefruitide-ref' -> 'extensions\grapefruitide-submodule\extensions\vscode'" -ForegroundColor White
...
$modulePath = Join-Path -Path $currentDir -ChildPath 'extensions\grapefruitide-submodule'
$targetPath = Join-Path -Path $modulePath -ChildPath 'extensions\vscode'
$linkPath = Join-Path -Path $currentDir -ChildPath 'extensions\grapefruitide-ref'
